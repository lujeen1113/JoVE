...
def ClusterComputing(
    ...
    for i in range(len(CHID_buff)):
    	...
    	for j in range(1,1+len(CHID_buff)):
    		if pos_matrix_temp[j][0]==0 and pos_matrix_temp[j][1]==0:
    			indx=j
    			break
    ...
    for i in range(1):
        ...
        if numpy.all(adjacency_matrix_buff[i] == 0) != True and edge_count >2:
            path = SolveBundle(flowm, pos_matrix_temp,
                               adjacency_matrix, opt, dest_id)
            print('solved solution is:'+str(path))                   
        elif numpy.all(adjacency_matrix_buff[i] == 0) != True and edge_count ==2:
...
def SpeedSlideWindow(opt):
    ...
    t_start = time.time()
    aus_num = 6
    whole_buff = numpy.zeros(aus_num*(aus_num+1))
    ...
    for i in range(segAmount):
        ...
        col=[0,0]
        pos_matrix_temp=numpy.insert(pos_matrix,0,col,axis=1)
        print(pos_matrix)
        numpy.append(pos_matrix,pos_matrix_temp,axis=0)
    ...
    while run_flag == True:
        ...
        while CH_Amount<max_graph_size:
        	for i in range(aus_num*(max_graph_size+1)):
        		if i%(aus_num+1)!=0:
        			if whole_buff[i]>0 and CHIdx_buff[i]==0:
        				if i % 2 == 0:
                        			E_start = 0.5
                    			else:
                        			E_start = 1
                    			...
                    			T_n_mnrm = P_bnrm * (u * (E_cur / E_start)+ v*(d_cur / d_max) ) / (1 - P_nrm* (count % (1 / P_nrm)))

                    			T_n_madv = P_badv * (u * (E_cur / E_start)+ v*(d_cur / d_max)) / (1 - P_adv* (count % (1 / P_adv)))
        ...
        chid_buff=CHID_buff
        while numpy.all(CHIdx_buff>0) ==False:
                CH_BUFF=[]
        	for i in range(len(chid_buff)):
        	    ch_buff=[]
        	    ID=chid_buff[i]
        	    ch_buff.append(copy.copy(ID))
        	    for j in range(aus_num*(max_graph_size+1)):
        	    	if j%(max_graph_size+1)!=0 and CHIdx_buff[j]==0:
        	    		x = pos_matrix[j][0]
                    		y = pos_matrix[j][1]
                    		...
                    		for p in range(len(CHID_buff)):
                        		...
                        		x_ch = pos_matrix[ID][0]
                        		y_ch = pos_matrix[ID][1]
        	    	...
        	    	if CH_ID=i and len(ch_buff)>=max_graph_size:
        	    		CHIdx_buff[j]=i
        	    		ch_buff.append(copy.copy(j))
        	    
        	    if len(ch_buff)>=max_graph_size:
        	    	numpy.delete(chid_buff,i)	
        ...
        for i in range(segAmount):
        	num=len(CH_BUFF[i])
        	adjacency_matrix=numpy.zeros((num,num))
        	for j in range(1,num):
        		adjacency_matrix[0][j]=1
        	...
        	(whole_buff_update, sol_bundle) = ClusterComputing(opt,whole_buff,pos_matrix,adjacency_matrix,0,CH_BUFF[i])
