...
def SpeedSlideWindow(opt1,opt2,sectIncrAmount):
 ...
 #for each node within each sector, assign the energy level 1 or .5 
    for i in range(aus_num):
    	#ene_buff to hold the node's energy level
        ene_buff = numpy.ones(aus_num+addon)
        #round_b to hold the number of times the node has been selected as a cluster head
        round_b = numpy.zeros(aus_num+addon)
        #id_buff to hold the cluster head node ID assigned to the node
        ...
        #create the loop to assign the energy level
        if i>0:
        	for j in range(aus_num+addon):
        		#for node sequence number as even, assign energy level 0.5
            		if j % 2 == 0:
                		ene_buff[j] = 0.5
                			#for node sequence number as odd, assign energy level 1
            		else:
                		ene_buff[j] = 1
    ...
    while run_flag == True and count==0:
        ...
        #for each node within each sector assign the energy value either 1 or 0.5 joul randomly
        for i in range(aus_num):
