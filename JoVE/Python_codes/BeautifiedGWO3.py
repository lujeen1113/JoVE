...
def segGraph_update(
    ...
    for i in range(max_graph_size):
    	if whole_buff[segNo][i]>0:
    		...
    		ene=ene+whole_buff[segNo][i]
    ene_ave=ene/valid_count
    ...
    for i in range(max_graph_size):
        ...
        if whole_buff[segNo][i] > 0:
            ...
            #if temp_dist <= dist and temp_ene>ene_ave:
    ...
    a=0
    for i in range(max_graph_size):
        if whole_buff[segNo][i] == ene_val_finir and dist_array[i] \
            == dist_val_finir:
            a=1
...
def SpeedSlideWindow(opt):
    ...
    while run_flag == True:
        ...
        for i in range(segAmount):
            ...
            adjacency_matrix_buff.append(copy.copy(adjacency_matrix))
            destID[i] = int(ID)
        for i in range(len(destID)):
        	ch_id=int(destID[i])
        	ch_x=pos_matrix_buff[i][0][ch_id]
        	ch_y=pos_matrix_buff[i][1][ch_id]
        	dist=math.sqrt(ch_x*ch_x+ch_y*ch_y)
        	if dist<87.6:
        		ene=bitsnum*E+epson_fs*bitsnum*dist*dist
        	else:
        		ene=bitsnum*E+epson_mp*bitsnum*dist*dist*dist*dist
        	whole_buff_update[i][ch_id]=whole_buff_update[i][ch_id]-ene 
        	
        	if whole_buff_update[i][ch_id]<0:
        		whole_buff_update[i][ch_id]=-1
