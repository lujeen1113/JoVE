...
def ClusterComputing(
    ...
    for i in range(1):
        ...
        if numpy.all(adjacency_matrix[i] == 0) != True \
            and edge_count > 2:
           #  call the SolveBundle to compute the source traffic dynamic (flowm) to the destination node
...
def SpeedSlideWindow(opt):
    ...
    for i in range(aus_num * (max_graph_size+addon + 1)):
        if i % (max_graph_size+addon+1) == 0:
            whole_buff[i] = -1
        elif (i - 1) % 2 == 0:
            whole_buff[i] = 0.5
        else:
    ...
    pos_matrix=numpy.zeros((aus_num*(max_graph_size+addon+1),2))
    for i in range(segAmount):
        ...
        for j in range(max_graph_size+addon+1):
        	idx=i*(max_graph_size+addon+1)+j
    ...
    cl_buff = numpy.ones(aus_num * (max_graph_size+addon + 1)) * 60
    ...
    while run_flag == True and count==0:
        ...
        CH_Amount = 0
        CHIdx_buff = numpy.ones(aus_num * (max_graph_size +addon + 1))
        ...
        cl_flag=True
        for i in range(aus_num*(max_graph_size+addon+1)):
        	if i%(max_graph_size+addon+1)!=0 and whole_buff[i]>0:
        ...
        count2=0
        print('alive node amount is:'+str(count_num))
        min_CH_Amount=count_num/(max_graph_size)
        
        while CH_Amount < min_CH_Amount and count_num>(max_graph_size):
            count2=count2+1
            #print(count2)
            CHID_buff = []
            CHIdx_buff = numpy.zeros(aus_num * (max_graph_size+addon + 1))
            ...
            for i in range(aus_num * (max_graph_size +addon+ 1)):
                ...
                elif whole_buff[j]<0 and j%(max_graph_size+addon+1)!=0:
            	     CHIdx_buff[j]=66                
        ...
        #print('whole_buff loop is:'+str(whole_buff)) 
            ...
            for j in range(aus_num * (max_graph_size+addon + 1)):
                if j % (max_graph_size+addon + 1) != 0 and CHIdx_buff[j]== 0 and whole_buff[j]>0:
                   x = pos_matrix[j][0]
                   y = pos_matrix[j][1]
                   ...
                   #for each node (non cluster head), calculate its distances to each cluster head (the for loop)
                   ...
                   for p in range(len(CHID_buff)):
                   	...
                   	#decide whether the minimum distance cluster head to the node has exceeded its capacity, if not, assign the node to this cluster head
       ...
       # print('whole_buff:'+str(whole_buff))
        ...
        print(segAmount)
        segLen=numpy.zeros(segAmount)
        for i in range(segAmount):
        	perseglen=len(CH_BUFF[i])
        	segLen[i]=perseglen
        print('the node amount per cluster head is: '+str(segLen))
...
#call the sub function ClusterComputing to compute the routes from the sensor nodes to the sink per sector: CH_BUFF[i] indicates the ith sector's cluster
        ...
        for i in range(aus_num * (max_graph_size+addon +1)):
            if whole_buff[i] < 0:
                count1 = count1 + 1
#First node die
        if count1 >= 40 and FND_flag == False:
            FND_num = count
            FND_flag = True

            # break
#Half node die
        if count1 >= (99+aus_num) and HND_flag == False:
            HND_num = count
            HND_flag = True

            # break
#Last Node die
        if count1 == (198+aus_num) and LND_flag == False:
            LND_num = count
            break
            run_flag=False
