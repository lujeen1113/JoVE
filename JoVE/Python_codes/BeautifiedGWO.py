...
def segGraph_update(
    ...
    return adjacency_matrix,min_indx
...
def ClusterComputing(
    ...
    whole_buff,
    pos_matrix_buff,
    adjacency_matrix_buff,destID,segAmount
    ):
    max_graph_size = 6
    theta_core = [
        30,
    ...
    E = 50*0.000000001
    epson_fs = 0.0000000010*10*0.001
    ...
    for i in range(segAmount):
        ...
        if numpy.all(adjacency_matrix_buff[i]==0)!=True:
            path = SolveBundle(flowm, pos_matrix_buff[i], adjacency_matrix_buff[i],
                           opt,dest_id)
        else:
            single=False
            count=0
            ...
            dist = math.sqrt((pos_matrix_buff[i][0][ID]- 0)* (pos_matrix_buff[i][0][ID]- 0)+ (pos_matrix_buff[i][1][ID]- 0)* (pos_matrix_buff[i][1][ID]- 0))

            if dist<=87.7085:
                whole_buff[i][ID]=whole_buff[i][ID]-bitsnum*E-bitsnum*epson_fs*dist*dist
            else:
                whole_buff[i][ID]=whole_buff[i][ID]-bitsnum*E-bitsnum*epson_mp*dist*dist*dist*dist
        ...
        for j in range(num):
            ...
            whole_buff[i][dest_id] = whole_buff[i][dest_id] \
                - bitsnum * E
            dist = math.sqrt((pos_matrix_buff[i][1][dest_id]
                                    - 0)
                                    * (pos_matrix_buff[i][1][dest_id]
                                    - 0)
                                    + (pos_matrix_buff[i][0][dest_id]
                                    - 0)
                                    * (pos_matrix_buff[i][0][dest_id]
                                    - 0))
            if dist<=87.7085:
                whole_buff[i][dest_id]=whole_buff[i][dest_id]-bitsnum*E-bitsnum*epson_fs*dist*dist
            else:
                whole_buff[i][dest_id]=whole_buff[i][dest_id]-bitsnum*E-bitsnum*epson_mp*dist*dist*dist*dist
...
def SpeedSlideWindow(opt):
    ...
    while run_flag == True:
        ...
        whole_buff_update,sol_bundle = ClusterComputing(opt, whole_buff,
                pos_matrix_buff, adjacency_matrix_buff,destID,segAmount)
        ...
        for i in range(segAmount):
            ...
            for j in range(aus_num):
                #print(str(i)+' '+str(j))
                #print('current buff is:'+str(whole_buff_update))
                if whole_buff_update[i][j] <= 0:
                    #print('indx is:'+str(i)+str(j))
                    new_str=str(i)+str(j)
                    count_flag=True
                    for k in range(len(obsolete_idx)):
                        if new_str==obsolete_idx[k]:
                            count_flag=False
                    ...
                    if count_flag==True:
                        count1=count1+1
                        obsolete_idx.append(copy.copy(new_str))
                        half_count = half_count+1
                        all_count = all_count+1
                        #print('half_count is:'+str(half_count))
                        if half_count == 18:
                            HND_num = count
                            #run_flag=False
            ...
            #print('valid segAmount is:'+str(ndim))
        ...
        if segInvalid_flag==True and ndim>1:
        	last_idx=len(delete_ID)-1
        	idx=delete_ID[last_idx]
        	#print('this_buff is:'+str(whole_buff_update[idx]))
            #segAmount=segAmount-1
        	...
        	whole_buff_update=numpy.delete(whole_buff_update,idx,axis=0)
        ...
        for i in range(segAmount):
            ang = theta_bound[i]
            adjacency_matrix, ID = segGraph_update(i,
                    segAmount, theta_core_pos, ang, whole_buff_update,pos_matrix_buff[i])
