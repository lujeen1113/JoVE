...
def SpeedSlideWindow(opt,addon,HnDCount,LnDCount):
    rate = 80
    ...
    for i in range(segAmount):
        ...
        (pos_matrix1, adjacency_matrix, ID) = segGraph(i, segAmount,
                theta_core_pos, ang)
        name = '/home/lupae/Documents/Imperial_Work/IC_Campus/Documents/Proj2-JoVE/Python_codes/posdata_folder/posdata' + str(i) + '.txt'
    ...
    while run_flag == True:
        ...
        FnDCount = aus_num+max_graph_size+addon+1
        
        if count1 >= FnDCount and FND_flag == False:
            print('first node die')
            FND_num = count
            FND_flag = True

            #break

        if count1 >= (HnDCount+1+aus_num) and HND_flag == False:
            HND_num = count
            HND_flag = True

            #break

        if count1 == (LnDCount+aus_num) and LND_flag == False:
            LND_num = count
            #break
            run_flag=False
